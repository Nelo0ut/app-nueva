...
<!-- Content Header (Page header) -->
    ...
    <section class="content">
      <form action="{{ route('oficina.update', $oficina) }}" method="post" autocomplete="off" role="form" id="quickForm" class="needs-validation" novalidate>
        @csrf
        @method('PUT')
        <div class="row">
          <div class="col-md-6">
            <div class="card card-naranja">
              ...
              <div class="card-body">
                ...
                <div class="form-group">
                  <label for="entidad_id">Entidad Bancaria *</label>
                  <select class="form-control select2 select2-purple" data-dropdown-css-class="select2-purple" style="width: 100%;" required id="entidad_id" name="entidad_id">
                    <option value="">:: SELECCIONE ::</option>
                    @foreach ($entidads as $entidad)
                      <option value="{{ $entidad->id }}" @if ($oficina->entidad_id == $entidad->id) selected @endif>{{ $entidad->name }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="numero" name="numero" class="form-control valnumero" maxlength="3" required value="{{ old('numero', $oficina->numero) }}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="numeroplaza" name="numeroplaza" class="form-control valnumero" maxlength="4" required value="{{ old('numeroplaza', $oficina->numeroplaza) }}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="preftelefono" name="preftelefono" class="form-control valnumero" maxlength="3" value="{{ old('preftelefono', $oficina->preftelefono) }}">
                </div>
                <div class="form-group">
                  <label for="telefono1">Número telefónico 1</label>
                  <input type="text" id="telefono1" name="telefono1" class="form-control valnumero" maxlength="8" value="{{ old('telefono1', $oficina->telefono1) }}">
                </div>
                <div class="form-group">
                  <label for="telefono2">Número telefónico 2</label>
                  <input type="text" id="telefono2" name="telefono2" class="form-control valnumero" maxlength="8" value="{{ old('telefono2', $oficina->telefono2) }}">
                </div>
                <div class="form-group">
                  <label for="fax">FAX</label>
                  <input type="text" id="fax" name="fax" class="form-control valnumero" maxlength="8" value="{{ old('fax', $oficina->fax) }}">
                </div>
                <div class="form-group">
                  <label for="centraltelefonica">Central telefónica</label>
                  <input type="text" id="centraltelefonica" name="centraltelefonica" maxlength="8" class="form-control valnumero" value="{{ old('centraltelefonica', $oficina->centraltelefonica) }}">
                </div>
                <div class="form-group">
        ...
        <div class="row">
          <div class="col-12">
            <a href="{{ route('oficina.index') }}" class="btn btn-naranja ">Regresar</a>
...
<script>
  ...
  $(function () {
    ...
    $.ajax({
        type:'POST',
        url:"{{ route('getubigeoadmin') }}",
        data:{ depa: '{{ $ubigeodis[0]->coddepa }}' },
        beforeSend:function(){
            $('.preload').show();
        },
        success:function(data2){
            if(data2.length > 0){
                for (var i = 0; i < data2.length; i++) {
                    cmb1 += "<option value='"+data2[i].codprov+"'>"+data2[i].nombre+"</option>";
                }
                //llenamos el combo on
                $('#inputDistriPro').html(cmb1);
                $('#inputDistriPro').val('{{ $ubigeodis[0]->codprov }}');
                $.ajax({
                    type:'POST',
                    url:"{{ route('getubigeoadmin') }}",
                    data:{ depa: '{{ $ubigeodis[0]->coddepa }}', prov: '{{ $ubigeodis[0]->codprov }}' },
                    beforeSend:function(){
                        $('.preload').show();
                    },
                    success:function(data3){
                        if(data3.length > 0){
                            for (var i = 0; i < data3.length; i++) {
                                cmb2 += "<option value='"+data3[i].id+"'>"+data3[i].nombre+"</option>";
                            }
                            //llenamos el combo on
                            $('#inputDistriDis').html(cmb2);
                            $('#inputDistriDis').val(ubigeo_id);
                        }else{
                            swalWithBootstrapButtons.fire(
                            'Aviso',
                            'Por el momento no tenemos información para su petición.',
                            'info'
                            );
                        }
                    },
