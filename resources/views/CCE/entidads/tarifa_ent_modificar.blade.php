            ...
            <div class="card card-body" style="overflow-x: scroll">
                ...
                <table id="example2" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        @for ($i = 0; $i < count($tarifas); $i++)
                            <tr>
                                <form action="{{route('modificar_tarifa_entidad', ['id' => $tarifas[$i]->id])}}" method="POST" role="form" id="modificar{{$i}}"
                                    ...
                                    @if ($tarifas[$i]->tipoplaza==1)
                                        ...
                                        <td colspan="3" align="center"><input type="number" min="0" class="form-control" name="sol_cci_montofijo" id="sol_cci_montofijo_{{$i}}"
                                            value="{{ $tarifas[$i]->sol_cci_montofijo}}"></td>
                                    @else
                                        ...
                                        <td><input type="number" min="0" class="form-control" name="sol_cci_montofijomin" id="sol_cci_montofijomin_{{$i}}" value="{{ $tarifas[$i]->sol_cci_montofijomin}}"></td>
                                        <td><input type="number" min="0" class="form-control" name="sol_cci_montofijomax" id="sol_cci_montofijomax_{{$i}}" value="{{ $tarifas[$i]->sol_cci_montofijomax}}"></td>
                                    @endif
                                        <td>
                                    ...
                                    @else
                                        ...
                                        <td><input type="number" min="0" class="form-control" name="dol_cci_montofijomin" id="dol_cci_montofijomin_{{$i}}" value="{{ $tarifas[$i]->dol_cci_montofijomin}}"></td>
                                        <td><input type="number" min="0" class="form-control" name="dol_cci_montofijomax" id="dol_cci_montofijomax_{{$i}}" value="{{ $tarifas[$i]->dol_cci_montofijomax}}"></td>
                                    @endif
                                    <td>
                ...
                <div class="row">
                    <div class="col-md-12 mb-3 my-3">
                        <a href="{{route('tarifa.index')}}" class="btn btn-morado">Regresar</a>
                    </div>
                </div>
            </div>
        </div>            
